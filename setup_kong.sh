...
curl --silent https://raw.githubusercontent.com/Mashape/docker-kong/master/config.docker/kong.yml > /etc/kong/kong.yml || error "Could not pull down latest Kong config from GitHub"
...
docker run -d -v /var/lib/cassandra:/var/lib/cassandra -p 9042:9042 --name cassandra mashape/cassandra || error "Could not spin up Cassandra instance"

# Spin up new Docker container consuming the local config we just made
info "Spinning up new Kong container"
docker run -d -v /etc/kong:/etc/kong -p 8000:8000 -p 8001:8001 --link cassandra:cassandra --name kong mashape/kong || error "Could not spin up Kong instance"
